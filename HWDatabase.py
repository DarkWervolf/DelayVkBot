...
class HWdatabase:
    def __init__(self):
        self.database = []
        if os.path.exists(os.path.realpath(__file__)[:os.path.realpath(__file__).rfind("\\") + 1] + "hw_all.txt"):
            self.read(os.path.realpath(__file__)[:os.path.realpath(__file__).rfind("\\") + 1] + "hw_all.txt")
            self.deactivate_past()

    def add(self, hw: homework):
        self.database.append(hw)
        self.database.sort(key=self.custom_key)
        self.save(os.path.realpath(__file__)[:os.path.realpath(__file__).rfind("\\") + 1] + "hw_all.txt")
    ...
    def delete_by_num(self, n: int):
        for h in self.database:
            if h.num == n:
                ...
                if os.path.exists(os.path.realpath(__file__)[:os.path.realpath(__file__).rfind("\\") + 1] + filename):
                    os.remove(os.path.realpath(__file__)[:os.path.realpath(__file__).rfind("\\") + 1] + filename)
                return True
        return False
