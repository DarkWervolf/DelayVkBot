...
class Bot:
    ...
    def run(self):
        while True:
            for event in VkLongPoll(self.vk_session).check():
                if event.type == VkEventType.MESSAGE_NEW and event.to_me and event.text and event.from_user:
                    ...
                    if text == 'админ' and user_id in self.database_admin.get_database():
                        ...
                        self.send_message(user_id,
                                          'Введите номер и дату дедлайна дз в формате:\nнн дд.мм.гг а\nГде нн - '
                        ...
                        if int(str.strip(text)) in self.god_ids:
                            self.send_message(user_id, 'Этот человек очень важен для нас, мы не можем его удалить',
                                              self.keyboard_admin)
                        ...
                        self.postponement_database.add(delay(user_id, user_name,
                                                             self.database_hw.get_by_num(
                                                                 self.active_users[user_index][2])))
    ...
    def _add_id_to_file_(self, user_id: int, homework_num: int):
        ...
        with open(os.path.realpath(__file__)[:os.path.realpath(__file__).rfind("\\") + 1] + filename, 'a') as file:
    ...
    def _get_available_hwrks_(self, user_id):
        if not self.hw_none:
            ...
            for a in self.hw_available:
                ...
                if not os.path.exists(os.path.realpath(__file__)[:os.path.realpath(__file__).rfind("\\") + 1] + filename):
                    with open(os.path.realpath(__file__)[:os.path.realpath(__file__).rfind("\\") + 1] + filename, 'w') as file:
                        a = 0
                with open(os.path.realpath(__file__)[:os.path.realpath(__file__).rfind("\\") + 1] + filename, 'r') as file:
